...
def write_VESTA(vesta, displacements, qpoint_band, q_position, scale, nsteps ):
    ...
    for qpoint in range(0, nqpoint, nsteps) :
        
        os.chdir(os.getcwd())
        q_pos = '%.5f' %float(q_position[qpoint].split(',')[0])
        q_pos += '_%.5f' %float(q_position[qpoint].split(',')[1])
        q_pos += '_%.5f' %float(q_position[qpoint].split(',')[2])
        path = 'VESTA_FILES'
        path += '/%s' %q_pos
        if os.path.isdir(path):
            shutil.rmtree(path)
        ...
        for band in range(nbands) : 
            ...
            filename += '(%.5f)' %qpoint_band[qpoint][band]
            filename += '.vesta'
...
scale = 5
nsteps = int(input("===>>> Enter step size :: "))
write_VESTA(vesta, displacements, qpoint_band, q_position, scale, nsteps)
#-----------------------------------------------------------
# Rough
